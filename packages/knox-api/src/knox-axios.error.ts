...
class KnoxRequestError extends Error {
  code: number;
  ...
  constructor(code: number, message: string, data?: string) {
    super(message);
    this.code = code;
    this.name = 'SamsungKnox Request Error';
    this.data = data;
  }
}

export const handleKnoxError = (error: AxiosError<BaseKnoxErrorResponse>) => {
  ...
  if (ERROR_MAP[code]) {
    throw new KnoxRequestError(code, `${ERROR_MAP[code]}: ${message}`, data);
  } else {
